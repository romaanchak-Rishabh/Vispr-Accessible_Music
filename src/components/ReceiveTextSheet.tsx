    ...
    for (const item of conflicts) {
      ...
      try {
        ...
        if (!audioBlob) {
          ...
          setImportProgress({ done, total: totalToImport, label: `Failed: ${inc.title} (no yt-dlp server)` });
          continue;
        }
        ...
        {phase === 'done' && (
          <>
            ...
            <div style={{ padding: '12px 16px', fontSize: 14 }}>
              ...
              {failed > 0 && <div style={{ color: '#ff3b30' }}>{failed} failed — no yt-dlp server configured</div>}
